...
public:
	...
	void SetMessageData(int effect, unsigned int color, vec_t *position, float fadein, float fadeout, float holdtime, float fxtime, const char *text);
	void SetSoundData(const char *name, float volume);
	void SetStatusData(int slots, int spectators, int proxies);
	void SetBannerData(const char *filename);
	void SetStuffTextData(const char *commands);
