...
NOXREF unsigned char *COM_LoadFileLimit(const char *path, int pos, int cbmax, int *pcbread, FileHandle_t *phFile);
unsigned char *COM_LoadHunkFile(const char *path);
unsigned char *COM_LoadTempFile(const char *path, int *pLength);
void COM_LoadCacheFile(const char *path, struct cache_user_s *cu);
NOXREF unsigned char *COM_LoadStackFile(const char *path, void *buffer, int bufsize, int *length);
