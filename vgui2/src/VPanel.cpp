...
void VPanel::Solve()
{
    ...
    if (IsPopup())
    {
        ...
        parent = (VPanel*)g_pSurfaceNext->GetEmbeddedPanel();
    }
...
void VPanel::SetVisible(bool state)
{
    ...
    g_pSurfaceNext->SetPanelVisible((VPANEL)this, state);

    _visible = state;

    if (IsPopup())
    {
        vgui2::g_pSurfaceNext->CalculateMouseVisible();
    }
}
...
{
    g_pSurfaceNext->MovePopupToFront((VPANEL)this);
